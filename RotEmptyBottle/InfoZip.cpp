...
//	seting file count
	...
	// ¶ªÆúÎÄ¼þÂ·¾¶by zdt [11/24/2006]
	zpParams.m_bJunkDir = TRUE;
...
//  seting file names
	for (int i=0; i<iFileCount; i++)
	{
		...
		strcpy_s(zpParams.m_pszFileNames[i], MAX_PATH+1,paFiles[i]);
	}
...
BOOL __stdcall DefaultZipCallback(CZipCallbackData *pData)
{
	...
	{
		QString pszErrorCode;
		pszErrorCode = QString("Zip error %1:<br>%2").arg(pData->m_liErrorCode).arg(pData->m_pszFileNameOrMsg);
		QMessageBox::critical(NULL, "critical", pszErrorCode, QMessageBox::Ok);		
	}
	return FALSE;
}
...
BOOL CInfoZip::InitializeZip()
{
	...
	if (GetInitializedUnzip())
		return TRUE;
	QString strName = "ZIPDLL.DLL";
	const wchar_t * encodedName = reinterpret_cast<const wchar_t *>(strName.utf16());
	m_ZipDllHandle = LoadLibrary(encodedName);
	if (!m_ZipDllHandle)
	{
...
BOOL CInfoZip::InitializeUnzip()
{
	...
	QString strName = "UNZDLL.DLL";
	const wchar_t * encodedName = reinterpret_cast<const wchar_t *>(strName.utf16());
	m_UnzipDllHandle = LoadLibrary(encodedName);
	if (!m_UnzipDllHandle)
	{
		SetLastError(ZERROR_DLL_NOT_FOUND);
		return FALSE;
...
{
	TCHAR pszCurrentDir[MAX_PATH+1];	
	ZeroMemory(pszCurrentDir,MAX_PATH+1);
	QString strName = pszTargetFolder;
	const wchar_t * encodedName = reinterpret_cast<const wchar_t *>(strName.utf16());
	GetCurrentDirectory(MAX_PATH+1, pszCurrentDir);
	SetCurrentDirectory(encodedName);
...
//  seting archive name
	...
	strcpy_s(uzpParams.m_pszArchiveFileName, strlen(pszArchive)+1,pszArchive);
...
//  seting file names
	...
	strcpy_s(uzpParams.m_pszFileNames[0],MAX_PATH+1, "*.*");
