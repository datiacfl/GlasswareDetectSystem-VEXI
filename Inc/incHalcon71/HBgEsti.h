...
class LIntExport HBgEsti: public HRootObject {
public:
  HBgEsti(void) {id = -1;}
...
private:
  ...
  HBgEsti(const HBgEsti &inst);
  HBgEsti &operator= (const HBgEsti &inst);
  // Internal Cleanup
  void Cleanup(void);

public: 
  // Tools
  const char *ClassName(void) const {return "HBgEsti";}
