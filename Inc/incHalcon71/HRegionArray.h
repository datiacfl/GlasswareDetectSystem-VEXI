...
public:
  HRegionArray(void) : array(NULL) {num_region_arrays++;}
  ...
  // Klassenfunktionen
  static Hlong NumInst(void) { return num_region_arrays; }
private:
  static Hlong     num_region_arrays; 
  HRegion  *array;
  void     NewLength(Hlong new_num, Hlong old_num);
  void     CopyHRegionArray(const HRegionArray &in);
public:
  static HRegionArray GrabRegionAsync(const Halcon::HTuple &FGHandle, const Halcon::HTuple &MaxDelay);
  static HRegionArray GrabRegionAsync(Hlong FGHandle, double MaxDelay);
  static HRegionArray GrabRegion(const Halcon::HTuple &FGHandle);
  static HRegionArray GrabRegion(Hlong FGHandle);
  ...
  virtual HRegionArray ProjectiveTransRegion(double HomMat2D, const char *Interpolation) const;
  virtual HRegionArray AffineTransRegion(const Halcon::HTuple &HomMat2D, const Halcon::HTuple &Interpolate) const;
  virtual HRegionArray AffineTransRegion(double HomMat2D, const char *Interpolate) const;
