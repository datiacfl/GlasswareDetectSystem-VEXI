...
typedef struct{
	...
	char          ProgLanguage[40]; /* prog. language of operator call   */
...
typedef struct {
	...
	INT4          imsize;           /* image format: max(Height/Width)   */
                                        /* (maxi_valid)                      */
        H_pthread_mutex_t mutex_ims;    /* mutex variable for imsize         */
                                        /* (only used in reference instance) */
	INT4          Width;            /* max. width of all images          */
                                        /* (maxi_valid)                      */
        H_pthread_mutex_t mutex_Wid;    /* mutex variable for Width          */
                                        /* (only used in reference instance) */
	INT4          Height;           /* max. height of all images         */
                                        /* (maxi_valid)                      */
        H_pthread_mutex_t mutex_Hei;    /* mutex variable for Height         */
                                        /* (only used in reference instance) */
	/*-------------------------------- region management ----------------*/
	INT4          CurrentRL;        /* current (max.) number of runs     */
                                        /* (maxi_valid)                      */
        H_pthread_mutex_t mutex_CurRL;  /* mutex variable for CurrentRL      */
                                        /* (only used in reference instance) */
        /*-------------------------------- processing context ---------------*/
        /*                                 ('set_system' or 'set_check')     */
	HBOOL         ClipRegion;       /* clipping at image (width/height)  */
                                        /* (write_back)                      */
        H_pthread_mutex_t mutex_CliReg; /* mutex variable for ClipRegion     */
                                        /* (only used in reference instance) */
        HBOOL         StoreEmptyRegion; /* segmentation results              */
                                        /* (write_back)                      */
        H_pthread_mutex_t mutex_StoEmpReg; /* mutex var. for StoreEmptyRegion*/
                                        /* (only used in reference instance) */
	HBOOL         InitNewImage;     /* set to zero before filtering      */
                                        /* (write_back)                      */
        H_pthread_mutex_t mutex_IniNewIma; /* mutex variable for InitNewImage*/
                                        /* (only used in reference instance) */
	UINT1         Neighbour;        /* Neighbourhood definition 4/8      */
                                        /* (write_back)                      */
        H_pthread_mutex_t mutex_Nei;    /* mutex variable for Neighbour      */
                                        /* (only used in reference instance) */
        /*----------------------------- operator-specific information -------*/
	HBOOL         DoIntZoom;        /* int/double coordinates in         */
	                                /* set_part, image_transform         */
                                        /* (write_back)                      */
        H_pthread_mutex_t mutex_DoIntZoo; /* mutex variable for DoIntZoom    */
                                        /* (only used in reference instance) */
...
typedef struct {
	...
	char          ReferenceDir[MAX_STRING]; /* PostScript documentation  */
        H_pthread_mutex_t mutex_RefDir;         /* m. var. for ReferenceDir  */
	char          HTMLDir[MAX_STRING];    /* HTML documentation          */
        H_pthread_mutex_t mutex_HTMLDir;      /* mutex variable for HTMLDir  */
	...
	HBOOL         OnlyLines;        /* for bug in X11R3 (IOx11DispRL)    */
        ...
        /*----------------------------- threadsafe HGlobalProcHandle usage --*/
        H_pthread_mutex_t mutex_globalhandle;  /* HGlobalProcHandle mutex v. */
...
/*---------------------------------------------------------------------------*/
/* H_LANG_INFO - data structure for propagating functions of the language    */
/* interface to the HALCON core/hlib.                                        */
/*---------------------------------------------------------------------------*/
typedef struct {
  void   (*DoError)(void);
  void   (*HSystem)(void);
