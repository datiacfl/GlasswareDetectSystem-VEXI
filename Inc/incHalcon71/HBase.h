...
/* the following structure is the 'mirror' of (=exactly the same as) the     */
/* HalconC.h-structure Htuple and only used to define the H_LANG_INFO        */
/* structure that contains language interface functions used by the core.    */
/* ATTENTION: Whenever changing the definition of Htuple, the Hmtuple        */
/* definition must be changed accordingly so that Htuple and Hmtuple keep    */
/* binary compatible!!                                                       */
typedef struct
{
  Hcpar   *tuple;         /* values (array von long/float/string)            */
  INT4_8  length;         /* length of array                                 */
} Hmtuple;
...
#define XY_IMAGE      (INT)64    /* Type: 2 INT1_IMAGE images (vectors)      */
#define COMPLEX_IMAGE (INT)128   /* Type: 2 float images                     */
#define LUT_IMAGE     (INT)256   /* Type: (0..255) with LUT (not implemented)*/
