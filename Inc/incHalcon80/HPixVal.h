...
//  HComplexPixVal(float Val): val(Val) {}
 ...
 // operator float(void) const { return (float)val; }
  ...
  HAbstractPixVal *Copy(void) const
  { HComplexPixVal *tmp = new HComplexPixVal((HComplex)val); return tmp; }
...
public:
  HPixVal(void) { val = new HBytePixVal(0); }
  HPixVal(const HComplex &Val) { val = new HComplexPixVal((HComplex)Val); }
  HPixVal(int Val) { val = new HInt4PixVal(Val); }
  HPixVal(Hlong Val) { val = new HInt4PixVal(Val); }
  HPixVal(HByte Val) { val = new HBytePixVal(Val); }
//  HPixVal(float Val) { val = new HFloatPixVal(Val); }
  HPixVal(double Val) { val = new HFloatPixVal(Val); }
  HPixVal(const HPixVal &Val) { val = Val.val->Copy(); }
