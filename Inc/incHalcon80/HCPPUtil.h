...
typedef enum { SINGLE_ARRAY } HBaseArrayMode;
...
public:
  HBaseArray(void) : num(0), num_max(0)   {} // EmptyArray()
  HBaseArray(Hlong n) : num(n), num_max(n) {} // NormalArray(n)
  HBaseArray(HBaseArrayMode m) : num(1), num_max(num_def) {} // SingleArray()
...
protected:
  ...
  void EmptyArray(void) {num = num_max = 0;}
  void SingleArray(void) {num = 1; num_max = num_def;}
  void NormalArray(Hlong n) {num = num_max = n;}
  void SetArray(Hlong n,Hlong nm) {num = n; num_max = nm;}
  Hlong AllocNum(void) const {return num_max;}
private:
  Hlong num;
  Hlong num_max;
  static  float incr;
...
public:
  ...
  HObject(Hkey key): used(TRUE), id(key) {}
  HObject(const Hobject &obj);
  // Copy-Konstruktor
  HObject(const HObject &r) {CopyHObject(r);}
...
public:
  ...
  HObjectArray(Hlong n) : HBaseArray(n) {}
  HObjectArray(HBaseArrayMode m) : HBaseArray(m) {}
...
{
  return HClipByte(r * 0.299 + g * 0.587 + b * 0.144);
}
