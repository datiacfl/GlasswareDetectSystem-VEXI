...
# define HAlloc(PROC_HANDLE,SIZE,VOID_PTR)                          \
  (HTraceMemory ?                                                   \
    HXAllocMemCheck(PROC_HANDLE,SIZE,"",(INT4_8)-1,H_GLOBAL_ALLOC,  \
                    (VOID_PTR)):                                    \
    HXAlloc(PROC_HANDLE,SIZE,(VOID_PTR)))                    
...
# define HRealloc(PROC_HANDLE,VOID_PTR,SIZE,NEW_PTR)                    \
  HXRealloc(PROC_HANDLE,VOID_PTR,SIZE,(NEW_PTR),"",(INT4_8)-1)
#else
# define HRealloc(PROC_HANDLE,VOID_PTR,SIZE,NEW_PTR)                    \
  HXRealloc(PROC_HANDLE,VOID_PTR,SIZE,(NEW_PTR),__FILE__,       \
            (INT4_8)__LINE__)
#endif
...
# define HFree(PROC_HANDLE,VOID_PTR)                                    \
  (HTraceMemory ?                                                       \
    HXFreeMemCheck(PROC_HANDLE,(VOID_PTR),"",(INT4_8)-1):               \
...
#define HAllocTmp(PROC_HANDLE,VOID_PTR,SIZE) \
   HXAllocTmp(PROC_HANDLE,(VOID_PTR),SIZE,"",(INT4_8)-1)
#else
#define HAllocTmp(PROC_HANDLE,VOID_PTR,SIZE) \
   HXAllocTmp(PROC_HANDLE,(VOID_PTR),SIZE,__FILE__,(INT4_8)__LINE__)
#endif
...
#define HAllocLocal(PROC_HANDLE,SIZE,VOID_PTR) \
   HXAllocLocal(PROC_HANDLE,SIZE,"",(INT4_8)-1,(VOID_PTR))
#else
#define HAllocLocal(PROC_HANDLE,SIZE,VOID_PTR) \
   HXAllocLocal(PROC_HANDLE,SIZE,__FILE__,(INT4_8)__LINE__,(VOID_PTR))
#endif

#ifdef SMALL
#define HReallocLocal(PROC_HANDLE,VOID_PTR,SIZE,NEW_PTR) \
    HXReallocLocal(PROC_HANDLE,VOID_PTR,SIZE,(NEW_PTR),"",(INT4_8)-1)
#else
#define HReallocLocal(PROC_HANDLE,VOID_PTR,SIZE,NEW_PTR) \
    HXReallocLocal(PROC_HANDLE,VOID_PTR,SIZE,(NEW_PTR),\
                   __FILE__,(INT4_8)__LINE__)
#endif
...
# define HReallocRLNumLocal(PROC_HANDLE,REGION,SIZE,NEW_REGION)         \
  HXReallocRLNumLocal(PROC_HANDLE,REGION,SIZE,NEW_REGION,"",(INT4_8)-1)
#else
# define HReallocRLNumLocal(PROC_HANDLE,REGION,SIZE,NEW_REGION)       \
  HXReallocRLNumLocal(PROC_HANDLE,REGION,SIZE,NEW_REGION,__FILE__,    \
                      (INT4_8)__LINE__)
#endif
...
#define Hisqrt(Val,Res) if (HROInfo.HSqrtTab == NULL)           \
  { HCkP(HAlloc(GLOBAL_ID,(size_t)(MAX_HSQRT * sizeof(HBYTE)),  \
                &(HROInfo.HSqrtTab)));                          \
...
/* ->LangIntID: char* */
#define HGetGV_LangIntID(PH)     (((Hproc_handle_)PH)->info_shared->lang_int_id)
#define HReadGV_LangIntID(PH,VAR)       (VAR=HGetGV_LangIntID(PH))
#define HWriteGV_LangIntID(PH,VAR)      (HGetGV_LangIntID(PH)=VAR)
...
#define HFOpen(FileName,Mode) HXFOpen(FileName,Mode,HROInfo.locale_filename_encoding)
#define HAccess(path,mode) HXAccess(path,mode,HROInfo.locale_filename_encoding)
#define HRemove(path) HXRemove(path,HROInfo.locale_filename_encoding)
#if defined(SGI) || defined(HPPA2) || defined(LINUX) || defined(SOLARIS) || defined(ALPHA) 
#define HStat(path,buffer) HXStat(path,buffer,HROInfo.locale_filename_encoding)
#define HLStat(path,buffer) HXLStat(path,buffer,HROInfo.locale_filename_encoding)
#define HOpenDir(name) HXOpenDir(name,HROInfo.locale_filename_encoding)
#endif
#define HUnlink(path) HXUnlink(path,HROInfo.locale_filename_encoding)
#define HOpen(filename,oflag) HXOpen(filename,oflag,HROInfo.locale_filename_encoding)
#ifdef _WIN32
#define HFindFirstFile(file_name,find_file_data) HXFindFirstFile(file_name,find_file_data, \
                                                                 HROInfo.locale_filename_encoding)
#define HFindNextFile(find_file,find_file_data) HXFindNextFile(find_file,find_file_data, \
                                                               HROInfo.locale_filename_encoding)
#endif
