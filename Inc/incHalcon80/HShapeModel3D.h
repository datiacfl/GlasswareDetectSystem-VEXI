...
class LIntExport HShapeModel3D: public HRootObject {
public:
  HShapeModel3D(void) {id = -1;}
  HShapeModel3D(
...
private:
  ...
  HShapeModel3D(const HShapeModel3D &inst);
  HShapeModel3D &operator= (const HShapeModel3D &inst);
  // Internal Cleanup
  void Cleanup(void);

public: 
  // Tools
  const char *ClassName(void) const {return "HShapeModel3D";}
