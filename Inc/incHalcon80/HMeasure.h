...
class LIntExport HMeasure: public HRootObject {
public:
  HMeasure(void) {id = -1;}
  HMeasure(
...
private:
  ...
  HMeasure(const HMeasure &inst);
  HMeasure &operator= (const HMeasure &inst);
  // Internal Cleanup
  void Cleanup(void);

public: 
  // Tools
  const char *ClassName(void) const {return "HMeasure";}
