...
void GlasswareDetectSystem::ReadIniInformation()
{
	...
	for (int i=0;i<=m_sErrorInfo.m_iErrorTypeCount;i++)
	{
		...
		else
		{
			...
			m_sErrorInfo.m_vstrErrorType.append(erroriniset.value(strSession, "NULL").toString());
	...
	for (int i=0;i<=StatusTypeNumber;i++)
	{
		...
		m_vstrPLCInfoType.append(PLCStatusiniset.value(strSession, "NULL").toString());//.toLatin1().data()));
	}
	//读取系统参数
...
void GlasswareDetectSystem::InitImage()
{
	...
	if (VexiFinishButtom == m_sSystemInfo.m_iSystemType  || VexiButtom == m_sSystemInfo.m_iSystemType)
	{
		...
		for(int i = 0 ; i < m_sSystemInfo.iRealCamCount; i++)
		{
			...
			m_sRealCamInfo[i].m_pRealImage = new QImage(m_sRealCamInfo[i].m_iImageWidth, m_sRealCamInfo[i].m_iImageHeight, QImage::Format_Indexed8);
		...
		for(int i = 0 ; i < m_sSystemInfo.iRealCamCount; i++)
		{
			...
			if (90 == m_sCarvedCamInfo[iCarvedCamNum].m_iImageRoAngle || 270 ==m_sCarvedCamInfo[iCarvedCamNum].m_iImageRoAngle)
			{
				...
				m_sCarvedCamInfo[iCarvedCamNum].m_pActiveImage = new QImage(m_sCarvedCamInfo[iCarvedCamNum].m_iImageHeight, m_sCarvedCamInfo[iCarvedCamNum].m_iImageWidth, QImage::Format_Indexed8);
			}
			else
			{
				//QT 5.12.6 update
				...
				m_sCarvedCamInfo[iCarvedCamNum].m_pActiveImage = new QImage(m_sCarvedCamInfo[iCarvedCamNum].m_iImageWidth, m_sCarvedCamInfo[iCarvedCamNum].m_iImageHeight, QImage::Format_Indexed8);
			}
...
void GlasswareDetectSystem::initInterface()
{
	...
	labelVersin->setText(tr("  V:1.32.1.9"));
...
void GlasswareDetectSystem::slots_ShowPLCStatus(int iStatus)
{
	...
	if (iStatus<m_vstrPLCInfoType.size())
	{
		...
		emit signals_HideWarning(2);
		emit signals_ShowWarning(8,m_sErrorInfo.m_vstrErrorType.at(iStatus-m_vstrPLCInfoType.size()-1)+QString::fromLocal8Bit("报警"));
	}
}
...
void GlasswareDetectSystem::InitCamImage(int iCameraNo)
{
	...
	for(int i = 0 ; i < m_sSystemInfo.iRealCamCount; i++)
	{
		...
		if (90 == m_sCarvedCamInfo[i].m_iImageRoAngle || 270 ==m_sCarvedCamInfo[i].m_iImageRoAngle)
		{
			...
			m_sCarvedCamInfo[i].m_pActiveImage = new QImage(m_sCarvedCamInfo[i].m_iImageHeight, m_sCarvedCamInfo[i].m_iImageWidth, QImage::Format_Indexed8);
		}
		else
		{
			//QT 5.12.6 update
			...
			m_sCarvedCamInfo[i].m_pActiveImage = new QImage(m_sCarvedCamInfo[i].m_iImageWidth, m_sCarvedCamInfo[i].m_iImageHeight, QImage::Format_Indexed8);
		}
