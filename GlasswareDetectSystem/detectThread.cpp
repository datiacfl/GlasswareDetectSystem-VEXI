...
bool DetectThread::isSendReject[256] = {false};
bool m_isSend1=false;
bool m_isSend2=false;
bool m_isSend3=false;
...
void DetectThread::kickOutBad(int nSignalNo)
{
	...
	if(grabImageCount == pMainFrm->m_sRealCamInfo[iCamera].m_iImageTargetNo)
	{
		...
		//CountDefectIOCard0(nSignalNo,m_lastResult);
		m_lastResult=0;
	}
}
	...
	if (cameraNumber == 1&&nGrabImageCount==1)
	{
		...
		else
		{
			...
			if (nCurTime-lastSpeed[cameraNumber] > 5000)
			{
				...
				//qDebug()<<"瓶子个数："<<nSpeedCount[cameraNumber]<<",时间："<<nCurTime-lastSpeed[cameraNumber]<<",速度："<<nCurSpeed;
...
{
	if(60<glasswareNum&&glasswareNum<100&&m_isSend1==false)
	{
		m_isSend1=true;
		glasswareNum = 80;
		m_isSend2=false;
		m_isSend3=false;
	}else if(140<glasswareNum&&glasswareNum<180&&m_isSend2==false)
	{
		m_isSend2=true;
		glasswareNum = 160;
		m_isSend1=false;
		m_isSend3=false;
	}else if(220<glasswareNum&&glasswareNum<255&&m_isSend3==false)
	{
		m_isSend3=true;
		glasswareNum = 240;
		m_isSend1=false;
...
void DetectThread::clearSendFlag()
{
	for (int i=0;i<256;i++)
	{
		isSendReject[i] = false;
