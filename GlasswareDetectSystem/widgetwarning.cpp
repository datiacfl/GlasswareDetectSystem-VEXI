...
#include <QLayout>
#include <QBitmap> //QT 5.12.6 update
	...
	: QWidget(parent)
{
	...
	btnStopWarning->setMask(pixmapBtnStop.mask());
...
void WidgetWarning::slots_StopAlert()
{
	...
	pMainFrm->test_widget->CloseAssert();
	pMainFrm->test_widget->kickOutTimer->stop();
	//QT 5.12.6 update
	//MaxNumber->start(60000,TRUE);
	MaxNumber->start(60000);
}
void WidgetWarning::slots_ReTim()
{
	pMainFrm->test_widget->slots_OKStatis();
}
