	...
	: QThread(parent)
{
	...
	QByteArray ba = m_strPLCInfoPath.toLatin1();
	const char* c_str = ba.data();
	strcpy(charPLCInfoPath, c_str);
...
void Widget_PLC::setLabelText()
{
	...
	for (int i=0;i<12;i++)
	{
		...
		listPLCInfoIN0.append(PLCiniset.value(strSession, tr("reserve")).toString());
	...
	for (int i=0;i<6;i++)
	{
		...
		listPLCInfoIN1.append(PLCiniset.value(strSession, tr("reserve")).toString());
	...
	for (int i=0;i<8;i++)
	{
		...
		listPLCInfoOUT0.append(PLCiniset.value(strSession, tr("reserve")).toString());
	...
	for (int i=0;i<4;i++)
	{
		...
		listPLCInfoOUT1.append(PLCiniset.value(strSession, tr("reserve")).toString());
