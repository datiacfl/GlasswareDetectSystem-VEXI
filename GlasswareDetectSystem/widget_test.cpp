	...
	: QWidget(parent)
{
	...
	checkBoxAll->setText(QString::fromLocal8Bit("全选"));
	connect(checkBoxAll,SIGNAL(stateChanged(int)),this,SLOT(slots_onStateChanged(int)));
 	...
 	labelMin->setText(QString::fromLocal8Bit("最大报警个数:"));
 	QLabel* labelMax = new QLabel(ui.widget_StatisErrorType);
 	labelMax->setText(QString::fromLocal8Bit("报警时间间隔(m):"));
...
//		iniStatisSet.setValue("/system/m_iIsTrackErrorType",1);	//报警类型
	...
	for (int i = 0;i<pMainFrm->m_sErrorInfo.m_iErrorTypeCount;i++)
	{
		...
		iniStatisSet.setValue(strSession,iStatisMaxTime[i]);	//报警比例
		strSession = QString("/TrackErrorType/MinRate%1").arg(i);
		iniStatisSet.setValue(strSession,iStatisMaxNumber[i]);	//报警比例
...
void WidgetTest::slots_StatisModeChanged(int index)
{
	...
	else if (2 == iStatisMode)
	{
		...
		ui.label_Number_2->setText(QApplication::translate("WidgetTest", "Number:", 0));
		ui.label_Number_2->setVisible(true);
		ui.spinBox_Number_2->setVisible(true);
		ui.pushButton_ShowErrorType->setVisible(true);
		kickOutTimer->stop();
	...
	else if (3 == iStatisMode)
	{
		...
		ui.label_36->setText(QString::fromLocal8Bit("报警最小值(%):"));
		ui.label_37->setText(QString::fromLocal8Bit("报警最大值(%):"));
