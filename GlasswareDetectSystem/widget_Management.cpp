...
void WidgetManagement::slots_load(bool bCurModel)
{
	...
	for(int i = 0; i < pMainFrm->m_sSystemInfo.iRealCamCount; i++)
	{
		...
		pMainFrm->m_sRealCamInfo[i].m_iTrigger=iniCameraSet.value(strTrigger,1).toInt();//默认外触发
	}
	for(int i = 0; i < pMainFrm->m_sSystemInfo.iRealCamCount; i++)
	{
		pMainFrm->m_mutexmGrab.lock();
		if(pMainFrm->m_sRealCamInfo[i].m_iGrabType == 0)
		{
			((CDHGrabberSG*)pMainFrm->m_sRealCamInfo[i].m_pGrabber)->SGSetParamInt(SGGrabSpeed,pMainFrm->m_sRealCamInfo[i].m_iShuter);
		}
		if(pMainFrm->m_sRealCamInfo[i].m_iGrabType == 8)
		{
			if(pMainFrm->m_sRealCamInfo[i].m_iTrigger == 1)
			{
				((CDHGrabberMER*)pMainFrm->m_sRealCamInfo[i].m_pGrabber)->MERSetParamInt(MERSnapMode,1);
				pMainFrm->m_sRealCamInfo[i].m_bGrabIsTrigger = true;
			}
			else if(pMainFrm->m_sRealCamInfo[i].m_iTrigger == 0)
			{
				((CDHGrabberMER*)pMainFrm->m_sRealCamInfo[i].m_pGrabber)->MERSetParamInt(MERSnapMode,0);
				pMainFrm->m_sRealCamInfo[i].m_bGrabIsTrigger = false;
			}
			((CDHGrabberMER*)pMainFrm->m_sRealCamInfo[i].m_pGrabber)->MERSetParamInt(MERExposure,pMainFrm->m_sRealCamInfo[i].m_iShuter);
		}		
