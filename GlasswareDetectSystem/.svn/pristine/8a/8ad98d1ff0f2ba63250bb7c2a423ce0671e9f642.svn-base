...
QExcel::QExcel(QString sTmpFilePath, QObject *parent,int OpenType)
{
	excel = 0;  
	workBooks = 0;  
	workBook = 0;  
	...
	{  
		...
		if (OpenType == 1)
		{
			workBooks->dynamicCall("Add(Object)",sTmpFilePath);
		}
		else
		{
			workBooks->dynamicCall("Open(const QString&)",sTmpFilePath);
		}
...
int QExcel::LoadTempleFile(QString sFilePath)
{
	...
	workBooks->dynamicCall("Add(Object)",sFilePath);
	return 0;
}
...
void QExcel::saveAs(QString sSaveFilePath)
{
	//QFile file(sSaveFilePath);
	//if (!file.exists())  
	//{
	//	file.open(QIODevice::WriteOnly);  
	//	file.close(); 
	//}
	workBook->dynamicCall("SaveAs(Object)",sSaveFilePath);  
}
